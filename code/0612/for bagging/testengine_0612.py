...
# import geffnet
    #train_model(model, dataloaders, criterion, optimizer, scheduler, device, num_epochs=25, is_inception=False)
def train_model(model, dataloaders, criterion, optimizer, scheduler, device,  lr_rate ,num_epochs=25, is_inception=False,):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['test']:
            ...
            for inputs in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if is_inception and phase == 'train':
                        ...
                        #outputs, aux = model(inputs) #inception會回傳兩個值，用其他的model就不用加aux,發現邱有寫了！
                ...
                # with open('./model output csv/vgg16-test-result.csv', 'a', newline='') as csvfile:  #######################################
                #         writer = csv.writer(csvfile)
                #         writer.writerow(['predict'])
                for i in range(len(preds)):
                    #print('preds=',preds[i])
                    #print('labels data=',labels.data[i])
                    with open('./model output csv/vgg16-test-result.csv', 'a', newline='') as csvfile:  #####################################
                        writer = csv.writer(csvfile)
                        writer.writerow([label_dict[preds[i]]])
            ...
            #畫出來:
            if phase == 'train':
                epoch_train_loss_history.append(epoch_loss)
            elif phase == 'val':
                epoch_val_loss_history.append(epoch_loss)
            ...
            if phase == 'val':
                ...
                early_stop(epoch_loss, lr_rate)

            if early_stop.early_stop == True:
                #break                         #這樣只會break 一層迴圈,還是會繼續跑...
                print('already early stopping！')
                time_elapsed = time.time() - since
                print('Training complete in {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60))
                #print('Best val Acc: {:4f}'.format(best_acc))
                model.load_state_dict(best_model_wts)
                return model, val_acc_history, best_acc, epoch_train_loss_history, epoch_val_loss_history
                
        #scheduler.step() #好像已有調整lr
        #print(optimizer.param_groups[0]['lr'])  ###check lr 有下降!
        print()
