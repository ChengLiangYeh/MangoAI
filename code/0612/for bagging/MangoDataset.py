...
class Mango(Dataset):
	def __init__(self, root='MangoData', data='train', mask=True, transform=None):
		...
		if data == 'train':
			...
			csv_pth = os.path.join(data_root, 'test.csv')
			self.image_root = os.path.join(data_root, self.maskroot + 'Test')
		else:
...
def get_mango_dataset(data='train', mask=True, transform='default'):
	if transform != 'default':
		...
		if data == 'train':
#0612transforms.RandomResizedCrop(224)
			transform = [transforms.Resize([224,224]), transforms.RandomRotation(15), transforms.RandomHorizontalFlip(),transforms.RandomVerticalFlip()]
			...
			data_transform = transforms.Compose([
				...
				#transforms.Grayscale(num_output_channels=3), #0612跑pt
				#transforms.RandomGrayscale(p=0.5),
