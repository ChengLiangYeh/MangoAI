...
import numpy as np
import time
from feature_blend_loader import *
from model import *
...
class featureNet(torch.nn.Module):
    def __init__(self,f_num):
        ...
        self.mobilenet = initialize_model('mobilenet', 3, False, use_pretrained=True)
        self.linear2 = torch.nn.Linear(f_num, 3)
    ...
    def forward(self, x,f):
        ...
        pred_1 =self.mobilenet(x)
        pred_2 =self.linear2(f)
...
,"all .pt/resnextdev_0611.pt","all .pt/vgg11dev_0611.pt","all .pt/vgg16dev_0611.pt","all .pt/squeezenetdev_0611.pt","all .pt/shufflenetdev_0611.pt",
"all .pt/resnextclache_dev_0612.pt","all .pt/squeezenetclache_dev_0612.pt","all .pt/squeezenetallgray_dev_0612.pt"]

transform = transforms.RandomRotation(15), transforms.RandomHorizontalFlip(),transforms.RandomVerticalFlip()
...
dataset = Mango(transform =data_transform)
...
dataloaders = {'train':dataloader_train, #0613
                    'val':dataloader_test}

optimizer = optim.SGD(model.parameters(), lr=lr_rate, momentum=0.9,weight_decay=0.1)
