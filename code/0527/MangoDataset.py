...
class Mango(Dataset):
	def __init__(self, data='train', typetype='train', transform=None):
		root = 'C1-P1_Train Dev_fixed/'
		csv_file = ''
		self.image_root = ''
		if data == 'train':
			csv_file = root + 'train.csv'
			self.image_root = root + 'C1-P1_Train/'
		elif data == 'dev':
			csv_file = root + 'dev.csv'
			self.image_root = root + 'C1-P1_Dev/'
		else:
			print('fail to load data')

		self.data = pd.read_csv(csv_file)
		if typetype = 'train':
			self.data = data[0:4760]
		elif typetype = 'val'
			self.data = data[4760:]
		else:
			pass
